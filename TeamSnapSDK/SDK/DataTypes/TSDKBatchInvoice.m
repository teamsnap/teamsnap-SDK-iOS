...
- (CGFloat)percentPaid {
    if (self.amountInvoiced == nil || [self.amountInvoiced compare:NSDecimalNumber.zero] == NSOrderedSame) {
        return 0.0;
    }
...
+ (void)createInvoicesWithDueDate:(NSDate *_Nonnull)dueDate
    ...
    if (createInvoiceCommand) {
        createInvoiceCommand.data[@"due_at"] = [dueDate yearMonthDateFormatter];    // We're intentionally only sending the year/month/date here. The BE will perform some conversions on it to make it due at 11:59:59 pm in the team's timezone.
